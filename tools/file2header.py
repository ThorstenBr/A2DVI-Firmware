...
def convertFile(inFile):
	...
	if FileHeader!=None:
		...
		out = ["const char "+FileName.replace(" ", "_")+"[] ="]
	if OutputType != "char":
		out[-1] += " {"
	LastLine = ""
	for e in l:
...
def run(args):
	global DoStrip, IsBinaryFile, Padding, XorChecksum
	global Arduino, FileHeader, BytesPerLine, OutputType
	...
	while len(args)>0:
		if args[0] == "--strip":
			...
			Padding = int(args[1])
			args = args[2:]
		elif args[0] == "--bytePerLine":
			BytesPerLine = int(args[1])
			args = args[2:]
		elif args[0] == "--outputType":
			OutputType = args[1]
			args = args[2:]
		elif args[0] == "--file":
			inFile = args[1]
